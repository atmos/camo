...
(function() {
  var Crypto, Fs, Http, Https, Path, QueryString, Url, accepted_image_mime_types, allowed_referers, camo_hostname, content_length_limit, current_connections, debug_log, default_security_headers, error_log, finish, four_oh_four, hexdec, logging_enabled, max_redirects, port, process_url, server, shared_key, socket_timeout, started_at, total_connections, version,
  ...
  process_url = function(url, transferredHeaders, resp, remaining_redirects) {
    ...
    if (url.host != null) {
      ...
      srcReq = Protocol.get(requestOptions, function(srcResp) {
        var contentType, contentTypePrefix, content_length, eTag, expiresHeader, is_finished, lastModified, newHeaders, newUrl, origin, referer;
        is_finished = true;
        ...
        if (content_length > content_length_limit) {
          ...
          if (!shared_key && (referer = srcResp.headers['referer'])) {
  ...
  server = Http.createServer(function(req, resp) {
    ...
    if (req.method !== 'GET' || req.url === '/') {
      ...
      if ((url.pathname != null) && dest_url && shared_key) {
      ...
      } else if (__indexOf.call(allowed_referers, '*') >= 0 || (_ref2 = req.headers.referer, __indexOf.call(allowed_referers, _ref2) >= 0)) {
        url = Url.parse(dest_url);
        return process_url(url, transferredHeaders, resp, max_redirects);
      } else {
