...
(function() {
  var Crypto, Fs, Http, QueryString, Url, camo_hostname, connect_timeout, current_connections, finish, four_oh_four, hexdec, log, logging_enabled, max_redirects, port, process_url, server, shared_key, started_at, total_connections, version;
  ...
  process_url = function(url, transferred_headers, resp, remaining_redirects) {
    ...
    if (url.host != null) {
      ...
      srcReq.setTimeout(connect_timeout * 1000, function() {
        srcReq.end();
        return four_oh_four(resp, "Timeout connecting to " + url.host);
      });
