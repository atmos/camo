...
(function() {
  var Crypto, Fs, Http, HttpProxyAgent, Https, HttpsProxyAgent, Path, QueryString, Url, accepted_image_mime_types, camo_hostname, content_length_limit, current_connections, debug_log, default_security_headers, error_log, finish, four_oh_four, hexdec, http_proxy, https_proxy, keep_alive, logging_enabled, max_redirects, port, process_url, server, shared_key, socket_timeout, started_at, total_connections, version,
  ...
  process_url = function(url, transferredHeaders, resp, remaining_redirects) {
    ...
    if (url.host != null) {
      ...
      if (url.protocol === 'https:' && https_proxy) {
        requestOptions['agent'] = new HttpsProxyAgent(https_proxy);
      } else if (url.protocol === 'http:' && http_proxy) {
